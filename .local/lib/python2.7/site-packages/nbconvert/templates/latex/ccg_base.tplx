...
((* block header *))
    ...
    \usepackage[sfdefault,scaled=0.85]{FiraSans}     % Basic figure setup, for now with no caption control since it's done
    \usepackage[small,euler-digits]{eulervm}         % automatically by Pandoc (which extracts ![](path) syntax from Markdown).
    \usepackage{textcomp}                            \usepackage{graphicx}
    \usepackage[mono,varqu,varl,scaled=0.95]{zi4}    % We will generate all images so they have a width \maxwidth. This means
